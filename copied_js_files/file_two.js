...
/*
Stuff here will attempt to allow the code to run in Node JS at least for the purposes of the stuff...
*/
// console.log(Sys)
Sys = {
    WebForms: {
...
Sys.WebForms.PageRequestManager.prototype = {
    ...
    _doPostBack: function (eventTarget, eventArgument) {
        var f = window.event; //undefined
        if (!f) {
            var d = arguments.callee ? arguments.callee.caller : null; // arguments = Arguments(2) ["ctl00$m$g_f6c765ee_3151_4000_aef7_ee0b51cfbe3c$ctl00$cboLocations30", ""]
            if (d) {
                var j = 30;
            ...
            // f = "ctl00$m$g_f6c765ee_3151_4000_aef7_ee0b51cfbe3c$ctl00$cboLocations30" at this point.
        }
        this._additionalInput = null;
        var h = this._form; // h = form#aspnetForm (html object???) (object with id #aspnetForm)
        if (eventTarget === null || typeof eventTarget === "undefined" || this._isCrossPost) { // false, skip this section.
            this._postBackSettings = this._createPostBackSettings(false);
            this._isCrossPost = false
        } else {// START HERE: 
            var c = this._masterPageUniqueID // "ctl00"?? not sure how to get this value
                , l = this._uniqueIDToClientID(eventTarget) // ctl00_m_g_f6c765ee_3151_4000_aef7_ee0b51cfbe3c_ctl00_cboLocations30, result of _uniqueIDToClientID
                , g = document.getElementById(l); // The <select> object we want to check.
            if (!g && c) // False
                if (eventTarget.indexOf(c + "$") === 0)
                    g = document.getElementById(l.substr(c.length + 1));
            if (!g) //False
                if (Array.contains(this._asyncPostBackControlIDs, eventTarget))
                    this._postBackSettings = this._createPostBackSettings(true, null, eventTarget);
                else if (Array.contains(this._postBackControlIDs, eventTarget))
                    this._postBackSettings = this._createPostBackSettings(false);
                else {
                    var e = this._findNearestElement(eventTarget);
                    if (e)
                        this._postBackSettings = this._getPostBackSettings(e, eventTarget);
                    else {
                        if (c) {
                            c += "$";
                            if (eventTarget.indexOf(c) === 0)
                                e = this._findNearestElement(eventTarget.substr(c.length))
                        }
                        if (e)
                            this._postBackSettings = this._getPostBackSettings(e, eventTarget);
                        else {
                            ...
                            function i(b) {
                                b = b ? b.toString() : "";
                                return m.test(b) && b.indexOf("'" + eventTarget + "'") !== -1 || b.indexOf('"' + eventTarget + '"') !== -1
                            }
                            if (b && (b.name === eventTarget || i(b.href) || i(b.onclick) || i(b.onchange)))
                                this._postBackSettings = this._getPostBackSettings(b, eventTarget);
                            else
                                this._postBackSettings = this._createPostBackSettings(false)
                        }
                    }
                }
            else // Pick it back up here: 
                this._postBackSettings = this._getPostBackSettings(g, eventTarget) // results in {async: false, asyncTarget: undefined, panelsToUpdate: undefined, sourceElement: undefined} 
        }
        if (!this._postBackSettings.async) { // True
            h.onsubmit = this._onsubmit; // A function of some sort, no longer sure about this...
            // Update: This points to one of the injected scripts from the Home.aspx file, trying to parse
            /** 
             * //<![CDATA[
             ...
             * 
            */
            this._originalDoPostBack(eventTarget, eventArgument); // ????????? IDK
            h.onsubmit = null; // cleanup
            return
        }
        h.__EVENTTARGET.value = eventTarget;
        h.__EVENTARGUMENT.value = eventArgument;
        this._onFormSubmit()
    },
...
console.log("Test")
console.log(Sys.WebForms.PageRequestManager.prototype._doPostBack())